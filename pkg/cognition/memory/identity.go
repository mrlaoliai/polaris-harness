...
// 用户层从 ~/.polaris-harness/config/prompts/identity.md 加载（优先级更高）。
...
//
// 优先级（高→低）：
//  1. 用户文件 ~/.polaris-harness/config/prompts/{name}（用户资产，DB 重置不影响）
...
// LoadSoulMD 从 ~/.polaris-harness/config/SOUL.md 加载用户自定义身份文件。
// 向后兼容旧路径，新路径为 ~/.polaris-harness/config/prompts/identity.md。
...
func LoadSoulMD() string {
	...
	b, err := os.ReadFile(filepath.Join(home, ".polaris-harness", "config", "SOUL.md"))
	if err != nil {
		return ""
	}
...
// WriteUserPrompt 将用户编辑的提示词写入 ~/.polaris-harness/config/prompts/{name}。
...
func WriteUserPrompt(name, content string) error {
	...
	dir := filepath.Join(home, ".polaris-harness", "config", "prompts")
...
func DeleteUserPrompt(name string) error {
	...
	path := filepath.Join(home, ".polaris-harness", "config", "prompts", name)
	err = os.Remove(path)
...
func loadUserPromptFile(name string) string {
	...
	b, err := os.ReadFile(filepath.Join(home, ".polaris-harness", "config", "prompts", name))
	if err != nil {
		return ""
	}
	return strings.TrimSpace(string(b))
