...
接口定义见 `internal/protocol/interfaces.go` (Provider)，类型定义见 `internal/protocol/types.go` (InferRequest/InferResponse/StreamEvent/ProviderCapabilities/TokenizerAdapter)。
...
- **KV Cache 路由与注入**: 针对支持 Prompt Caching 的 Provider（如 Anthropic/DeepSeek），Adapter 在反序列化请求时，基于底层块的静态顺序属性，自动在稳定长文本块结尾注入 `cache_control: {"type":"ephemeral"}`。此逻辑与 M5 的 ContextAssembler 组装顺序（ImmutableCore → Procedural → Episodic）紧密耦合，确保最大化复用 KV Cache。
