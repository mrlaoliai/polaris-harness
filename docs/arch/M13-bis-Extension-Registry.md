...
| 表 | 迁移文件 | 消费方 |
|----|---------|-------|
| `plugin_marketplaces` | 024 | M13 API |
| `registry_cache` | 029/033 | M13 API |
| `extension_instances` | **034**（新增） | M13 API（SSoT） |
| `mcp_servers` | 018/019/022 | M7 MCPManager |
| `skills` | 008/021/026废弃 | M6 SkillRegistry |

**已废弃**：`skill_sources`（023）、`plugins`（027）、`apps`（028）、`026_skills.sql`（死代码）。迁移脚本见 `035_deprecate_install_tables.sql`。
