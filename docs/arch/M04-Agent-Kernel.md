...
| inv_M4_02 | 重放时不重新调 LLM——用 EventLog 录像值（请求全文 + 响应全文） | M4 §8 ReplayMode 物理切断 (TODO: 待实现) |
...
**回放机制**: M4、M5、M11 启动时统一检查 [ReplayMode] 标志 (TODO: 待实现)（进程级 atomic.Bool）。回放期间禁止所有外部副作用（EmitEvent/ToolCall/Outbox）——纯函数式重建内存状态。追平事件流后退出回放模式，从崩溃点继续执行。
...
**双重幂等防线**: 第一层 isReplaying 标志 (TODO: 待实现) 物理切断副作用；第二层 UNIQUE(session_id, seq) 约束 + idempotency_key 保证重复事件的幂等消费。
