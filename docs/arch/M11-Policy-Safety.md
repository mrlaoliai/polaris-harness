...
Layer 2 硬约束 — Cedar forbid (无条件优先 permit):
  ...
  forbid: Holdout Set 读取隔离 → principal in Role::"Agent" AND action in [Action::"read_local", Action::"read_file"] AND resource.path.startsWith(context.polaris-harness_eval_holdout_path)
    说明: context.polaris-harness_eval_holdout_path 由 Go 侧在策略加载时注入（展开后的绝对路径，等价于 ~/.polaris-harness/eval/holdout/）。ci_gate role 不受此 forbid 限制（CI/Canary 需要读取 Holdout Set）。此规则为防御纵深——物理隔离层（WASI 沙箱 + Openat2 RESOLVE_IN_ROOT）已阻止逃逸，Cedar 规则覆盖 Host Function 层可能的访问向量。
...
executeFullStop:
  步骤1: 写入 ~/.polaris-harness/.fullstop (timestamp + reason + actor, 0600)
...
Sealed 模式启动检查 (main 最早执行):
  步骤1: 检查 ~/.polaris-harness/.fullstop 存在
...
| ~/.polaris-harness/KILLSWITCH 文件 | fsnotify 监视, 存在 → Full Stop | <500ms |
...
KILLSWITCHFileWatch: fsnotify 监视 ~/.polaris-harness/KILLSWITCH, 存在 → executeFullStop, 删除后恢复
...
SessionPIIVault:
  ...
  **跨会话挂起持久化（解决 Suspended 任务 PII 丢失）**:
  SuspendSnapshot(ctx, taskID):
    ...
    ≤4KB → tasks.pii_vault_blob 列 (base64)；>4KB → VFS blob (`~/.polaris-harness/vfs/`) + tasks.vfs_ref (VFS refcount+1)
