...
> **§跳读**: 0-bis:7 职责 / 0-ter:18 不变量速查 / 1:29 四层映射 / 2:38 L0 Working / 3:125 L1 Episodic / 4:219 L2 Semantic / 5:240 L3 Procedural / 6:250 写路径 / 7:262 HybridRetriever / 8:363 EffConn / 9:373 Consolidation / 10:391 Forgetting / 11:407 ContextAssembler / 12:436 Drift / 14:474 496(SOFT)降级 / 15:496 依赖
## 0-bis. 职责边界
...
EvidenceSubgraphExtractor:
  graphDB, maxDepth=`spec/state.yaml §m5_memory.graph_max_depth`, maxNodes(50), alpha(0.85)
Extract:
...
**4-Stage Pipeline** (`pkg/cognition/consolidation.go`):

- Stage 1 — **实体/关系提取**: 聚合 Session 内 Episodic 事件文本，优先调用 Provider LLM 提取命名实体与关系（JSON 结构化输出），LLM 不可用时降级为正则模式匹配（CamelCase 概念 / 文件路径 / URL / 常量），共现关系由相邻实体对推断。
- Stage 2 — **Upsert Semantic**: 批量调用 `SemanticMemory.UpsertFact / UpsertRelation`；同一实体 `SyncVersion` LWW 保留最新，冲突事实标记 superseded；新实体直接 INSERT。单条失败不中止批次。
...
`PeriodicCleanup` 扫描 `events:` 前缀；`PhysicalCompact` 扫描 `forgettable:` 前缀批量清理——两步分离保证单次扫描不阻塞写路径。
...
**用户自定义身份**：`~/.polaris-harness/config/SOUL.md` 存在时覆盖 `DefaultPolarisIdentity`，服务启动时一次性读取并缓存到 `Server.soulMDContent`。
...
| **Layer 1** 用户自定义 | `~/.polaris-harness/config/prompts/*.md` | 用户 | 直接编辑文件 或 `PUT /v1/config/prompts/{name}` | **存活**（文件不在 DB 中） | `DELETE /v1/config/prompts/{name}` 显式删除 |
