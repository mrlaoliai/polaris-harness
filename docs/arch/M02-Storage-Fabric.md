...
**Workspace 绝对生命周期上限防线（防永久泄漏）**:
  ...
  _KillSwitch-Suspended_: 无 TTL（等 unseal 自动恢复）。但磁盘 <100MB CRITICAL 且 workspace UpdatedAt >7 天 → 打包 `~/.polaris-harness/archive/<task_id>_<timestamp>.tar.zst` 删原目录，保留 Blackboard 元数据。unseal 时 M13 检查 archive 存在 → 先解压再恢复任务。归档上限 10GB（LRU 删最老 + WARN）。
...
VFS 引用计数 + SQLite Trigger 自动回收: 热表大型载荷 (>4KB) 不存入 B-Tree 页，写入 VFS 文件 (`~/.polaris-harness/vfs/{sha256_prefix}/{uuid}.blob`)，热表仅存 `vfs_ref` 指针。4KB 热行硬限防 B-Tree 页缓存血崩。
